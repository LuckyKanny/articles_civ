...
def app():
	...
	for link in links_test:
		...
		page_templates_url += f"{page_title}&prop=templates"
		...
		if (len(page_templates) > 0):
			...
			for template in page_templates:
				if template["*"] in modele_bandeau:
					...
					articles_count += 1
					print(f"{page_title} has some issues.")
	# Print total
	printNotice(f"In total, {str(articles_count)} articles have issues.")
	if (articles_count > 0):
